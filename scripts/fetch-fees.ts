...
import { fetchFees } from '../src/fetchers/fees.ts';
...
async function main(): Promise<void> {
  ...
  logger.info('start fetching fees', { total: codes.length, qps: QPS });
  ...
  await pool.run(
    ...
    async (code) => {
      ...
      try {
        const fees = await fetchFees(code);
        upsertFees(db, { ...fees, fundCode: code });
        writeFetchLog(db, {
          fundCode: code,
          source: 'eastmoney',
          endpoint: 'jjfl',
    ...
    (done, total) => {
      if (done % 500 === 0 || done === total) {
        ...
        logger.info('progress', { done, total, ok, failed, ratePerSec: rate, etaSec: eta });
      }
    },
  );
