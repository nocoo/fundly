...
- **精选门槛往返**：支持门槛开关（`feeOn`, `scaleOn`, `ddOn`, `toOn`, `peOn`, `roeOn`, `yoyOn`, `stOn`）与自定义数值（支持合法 `0`）的完整 URL/本地存储双向序列化；关闭条件后仍保留数值。输入框允许清空、负号和小数草稿，在失焦 / Enter 时提交，并应用该字段的最小 / 最大值；没有变化时不重复导航。
- **企业与行业切片**：股票列表提供「全部企业 (含金融)」、「排除已知金融股」、「仅看金融业 (银行/证券/保险)」三态切换；经营类视角默认排除不可比金融股，切换到金融专属切片时自动保留。
- **分页与结果计数**：每页 50 条，即使少于 1 页也展示总数与分页控件状态；页码限制到 1–10,000，避免小于 1 的小数或过大页码污染状态。ETF 已有费用 / 规模但尚无价格或净值序列时标为「资料」，不会归到「仅目录」。

ETF / 股票详情共用 `selection-details.css`：桌面五项摘要保持一行，手机为两列并让最后一项占满；主 K 线占约三分之二宽，高度为 540–720px，手机 520px。股票详情另有年度收入、归母净利润、经营现金流曲线与报表；缺数不补零，币种不同不连线，负现金流与零利润保留。

导航按当前路径与显式来源状态确定分组，直接打开新详情不读取其他资产组的旧来源。宏观行业进入行业估值比较，活跃成分进入股票详情；ETF 持仓仅对境内股票提供下钻，并保留「股票 → ETF 详情 → 原筛选列表」返回链。详情页区分 404、研究资料未就绪和请求失败，后两者可重试；只有 NAV 时不生成 K 线。
...
质量目标与采集层相同：Biome 零告警，核心逻辑单测。UI 新增 ViewModel 必须带测试；最近实测结果与未达目标的部分见文档 16。
